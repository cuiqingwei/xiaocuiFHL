...
/*----------------*
sleep.hÀïÃæ¶¨ÒåµÄ³£Êı£¬¶ÔÓ¦¸÷ÖÖË¯ÃßÄ£Ê½
#define SLEEP_MODE_IDLE         0   							¿ÕÏĞÄ£Ê½
#define SLEEP_MODE_ADC          _BV(SM0)  						ADC ÔëÉùÒÖÖÆÄ£Ê½
#define SLEEP_MODE_PWR_DOWN     _BV(SM1)						µôµçÄ£Ê½
#define SLEEP_MODE_PWR_SAVE     (_BV(SM0)|_BV(SM1))				Ê¡µçÄ£Ê½
#define SLEEP_MODE_STANDBY      (_BV(SM1)|_BV(SM2))				Standby Ä£Ê½
#define SLEEP_MODE_EXT_STANDBY  (_BV(SM0)|_BV(SM1)|_BV(SM2))	À©Õ¹StandbyÄ£Ê½
//
void set_sleep_mode (uint8_t mode);		Éè¶¨Ë¯ÃßÄ£Ê½
void sleep_mode (void);					½øÈëË¯Ãß×´Ì¬
 *----------------*/
...
void cpu_init(void) 
{
	/* ¶Ë¿ÚÅäÖÃ */
 	...
 	DDRD  = 0x72;
  	...
  	PORTD |= _BV(bitHALLPWR);	// ´ò¿ª»ô¶û´«¸ĞÆ÷  
	...
	{	
		...
		if (tLap == 0xffff)
		{
     		...
     		/* ¹Ø±ÕµÄÓĞLED¼°»ô¶û´«¸ĞÆ÷ */
      		...
      		/* ĞİÃß´ı»ú */
			set_sleep_mode(SLEEP_MODE_PWR_SAVE);	// Ê¡µçÄ£Ê½
			sleep_mode();
